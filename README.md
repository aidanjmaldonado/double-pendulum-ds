...
3. *Function* `runge_kutta_method.m` - Calculates the $$(n+1)$$-th state values of $$\theta_{1}$$, $$\dot{\theta_{1}}$$, $$\theta_{2}$$, $$\dot{\theta_{2}}$$ given them at the $$n$$-th state. Takes the weighted average of the intermediate slopes $$k_{1}$$ through $$k_{4}$$. Calls `M1AA.m` and `M2AA.m`.

4. *Function* `trapezoid_method.m` - Calculates the $$(n+1)$$-th state values of $$\theta_{1}$$, $$\dot{\theta_{1}}$$, $$\theta_{2}$$, $$\dot{\theta_{2}}$$ given them at the $$n$$-th state. Takes the average of the slopes at the start and end of each time interval. Calls `M1AA.m` and `M2AA.m`.
